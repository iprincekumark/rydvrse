...
import { DRIVER_SUPPORT_FAQS } from "@/constants/driverSupportFaq";
...
function composeAgentReply(faqId?: string, faqAnswer?: string) {
  if (faqAnswer) {
    return `${faqAnswer}\n\nLet me know if this resolves it, or share the trip ID and I'll escalate.`;
  }
  if (faqId === "custom") {
    return "Thanks for reaching out. Share the trip ID or a short description and I'll pull up the right details.";
  }
  return "Thanks for reaching out — a driver specialist will follow up here in a moment.";
}

/**
 * Driver-side simulated support chat. Mirrors the customer version but
 * sources replies from DRIVER_SUPPORT_FAQS so the canned answer matches the
 * FAQ the driver tapped.
 */
export function DriverSupportChatScreen() {
  ...
  const faq = useMemo(
    () => DRIVER_SUPPORT_FAQS.find((item) => item.id === seedFaqId),
    [seedFaqId],
  );
    ...
    const base: ChatMessage[] = [
      {
        id: "system-welcome",
        author: "system",
        body: "You are in the Rydvrse driver support queue. Typical wait under 2 minutes.",
    ...
    const connectTimer = setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
          id: "system-joined",
          author: "system",
          body: "Arjun from Rydvrse Driver Support joined the chat.",
    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
          ...
          author: "agent",
          body:
            "Got it — I have logged this against your driver profile. I'll stay on chat until we close it.",
          timestamp: new Date().toISOString(),
        },
      ]);
      setTyping(false);
  ...
  return (
    <Screen scrollable={false}>
      <ScreenHeader
        title={faq?.question ? "Driver support" : "Chat with Rydvrse"}
